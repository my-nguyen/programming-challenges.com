...
class skiena_1_6_5
{
  ...
  static void output(List<Command> list) throws FileNotFoundException
  {
...
class Command
{
  ...
  // to declare the member as List<Image>
...
class Image
{
  ...
  // the bitmap, and since Java String is immutable (can't be re-written),
  // char[] is chosen instead of String
  private List<char[]> bitmap;
  ...
  public String toString()
  {
    ...
    {
      for (int j = 0; j < bitmap.get(i).length; j++)
        builder.append(bitmap.get(i)[j]);
      builder.append("\n");
    }
    return builder.toString();
  }

  // Create a new M x N image with all pixels initially colored white 'O'
  ...
  // Clear the table by setting all pixels white '0'. The size remains unchanged
  public void clear()
  {
    for (int i = 0; i < bitmap.size(); i++)
      for (int j = 0; j < bitmap.get(i).length; j++)
        bitmap.get(i)[j] = WHITE;
  }

  // Return the color of the pixel at (column, row)
  char get(int column, int row)
  {
    return bitmap.get(row-1)[column-1];
  }

  // Colors the pixel (X, Y ) in color (C)
  void color(int column, int row, char couleur)
  {
    bitmap.get(row-1)[column-1] = couleur;
  }
  ...
  {
    if (row >= 1 && row <= bitmap.size() &&
      column >= 1 && column <= bitmap.get(0).length &&
      bitmap.get(row-1)[column-1] == old_color)
    {
      bitmap.get(row-1)[column-1] = new_color;
